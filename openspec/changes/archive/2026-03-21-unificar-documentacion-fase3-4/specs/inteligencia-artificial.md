...
2. **OCR y Parseo de Tickets:**
   - Envío de un payload binario (imagen de un ticket/factura).
...
- **Scenario: Escaneo de Ticket con Llama Local**
  - *Given* el usuario sube un archivo `.jpg` desde la webapp hacia el endpoint `/ia/ocr`, estando el sistema configurado para apuntar a un cluster local Llama.
